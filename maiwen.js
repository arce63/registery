...
client.on("guildMemberAdd", member => {
  ...
  const reglog = member.guild.channels.cache.find(r => r.id === "872804744186433596");
   ...
   reglog.send(`${member} - ( \`${member.id}\` ) sunucuya katıldı fakat hesabı \`saniyeler önce\` oluşturulduğu için Şüpheliye atıldı.`)
setTimeout(() => {
...
client.on("userUpdate", async (say, yeni) => {
  var sunucu = client.guilds.cache.get('872801707514540032'); 
  ...
  if (!(yeni.username).includes(ekipTag) && uye.roles.cache.has(ekipRolü)) {
    try {
      ...
      await uye.send(`Selam dostum tagımızı bıraktığın için çok üzüldük eğer tekrardan aramıza katılmak istersen buyur \`${ekipTag}\` **İYİ GÜNLER DOSTUM**`);
...
client.on("guildMemberAdd", member => {
  member.roles.add('872801707627786327'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});

client.on("guildMemberAdd", member => {
  member.setNickname("✮ İsim | Yaş") // GİRENİN İSMİNİ DEĞİŞTİRME
});

//---------------------TAG-İLE-KATILMA--------------------//

client.on("guildMemberAdd", member => {
  let sunucuid = settings.kanallar.guildID; 
...
client.on("guildMemberAdd", member => {
    ...
    let date = moment(member.user.createdAt)
     ...
     client.channels.cache.get(settings.kanallar.hgkanal).send(`:tada: Sunucumuza hoşgeldin! ${member}

Hesabını ${kuruluş} (${string}) önce oluşturmuşsun |  

Sunucu kurallarımız <#872801708521168969> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.
